...
> All IP and MAC addresses below are **placeholders**. Real lab addressing has been redacted.
...
3. Confirm your virtual network adapter attached to the pfSense LAN switch has an IPv4 address starting with `10.10.1.x` (other than `.1`).
4. If it shows an APIPA address (`169.254.x.x`), your virtual switch or DHCP is misconfigured.
5. Open a browser and navigate to the pfSense web GUI (typically `http://10.10.1.1`).

## Verification

Run `ipconfig` and verify the IPv4 Address falls within `10.10.1.2` to `10.10.1.254`. Otherwise the VM cannot reach the firewall's DHCP and the GUI will not be reachable.

## Interfaces

- WAN = hn0 (10.10.0.x - Hyper-V NAT, treated as internet)
- LAN = hn1 (10.10.1.0/24 - trusted)
- Guest Net / OPT1 = hn2 (10.10.2.0/24 - untrusted guests)

Adding an interface: Interfaces -> Assignments -> pick the NIC -> Add -> configure IPv4 static (e.g. 10.10.2.1/24) -> Save/Apply.

## Guest Network Setup

- Interface: hn2 assigned as OPT1, IP 10.10.2.1/24
- DHCP Server (Services -> DHCP Server -> Guest Net):
  - Enabled, Address Pool Range: 10.10.2.2 - 10.10.2.100
  - Gateway: 10.10.2.1, DNS: 1.1.1.1 / 8.8.8.8
  - Static Mapping: MAC 00:15:5D:XX:XX:XX -> 10.10.2.101 (guest-vm)
    - Note: pfSense requires static mapping IPs to be OUTSIDE the pool range
- Fedora guest VM: DHCP works, got 10.10.2.101. SSH enabled (sshd) for remote access.

## Firewall Rules

Rules are evaluated top-to-bottom, so the isolation blocks MUST come before the broad allow rule (otherwise a pass-any matches LAN-bound traffic first and the blocks never fire). Guest Net rules:

| # | Action | Protocol | Source | Destination | Port | Purpose |
|---|---|---|---|---|---|---|
| 1 | Block | any | Guest Net subnet | LAN subnet (10.10.1.0/24) | - | Isolate LAN |
| 2 | Block | any | Guest Net subnet | 10.10.0.0/24 | - | Block WAN-side lab net |
| 3 | Pass | UDP | Guest Net subnet | Firewall self | 67-68 | DHCP |
| 4 | Pass | TCP/UDP | Guest Net subnet | any | 53 | DNS |
| 5 | Pass | any | Guest Net subnet | any | - | Internet |
...
Verified: guest can reach internet (8.8.8.8 + gateway), LAN pings (10.10.1.1/.2) are blocked and logged.
...
- Peers: vr_iphone (10.10.10.2), vr-mac (10.10.10.3) - Allowed IPs -/32 + 10.10.1.0/24 + 10.10.2.0/24
...
- Peer: pfSense server public key, Endpoint 203.0.113.10:51820 (home public IP - redacted, shown here as a documentation-range placeholder), AllowedIPs 0.0.0.0/0 (full tunnel)
- "Exclude private IPs" toggle must be OFF

Home router (Xfinity app): Port forward UDP 51820 -> 10.10.0.10 (pfSense WAN).
...
- Or SMB share mount (//10.10.0.230/Users/vero/Documents via cifs).
...
5. Testing & verification - show evidence: ipconfig within 10.10.1.2-254, ping results, rule-blocked vs allowed traffic, system log entries proving a rule worked.
